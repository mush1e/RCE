...
void Filesystem_Manager::add_problem_dir(const std::string& problem_id) {
...
void Filesystem_Manager::delete_problem_dir(const std::string& problem_id) {
    std::string directory_name = this->base_directory + "/" + problem_id;

    if (std::filesystem::remove_all(directory_name.c_str()) == -1)
        std::cerr << "Error : " << strerror(errno) << " - " << problem_id << std::endl;
    else
        std::cout << "Directory deleted" << " - " << problem_id << std::endl;
}
